...
D_INIT = {}
STAGE_U_D = {}  # stage -> device
D_T_STAGE = {}  # device -> stage
D_P_M = {}
# 初始化工艺
for index, row in craft_pd.iterrows():
    ...
    craft = Craft(d_id=d_id, m_id=m_id, target=(MATERIAL[m_id], p), changeTime=cT, line_id= cLine)
...
def get_oder():
    ...
    order_craft = Craft(d_id='order', m_id='order', target=(orderML, 1), line_id='None', changeTime=0)
