...
from MDP.modules_vx import Material, Craft, Device, Stage
...
for index, row in craft_pd.iterrows():
    ...
    if m_id in M_T_STAGE:
        ...
        craft = Craft(d_id=d_id, m_id=m_id, target=(MATERIAL[m_id], p), changeTime=cT, line_id=m_id)

        if stage in STAGE_U_D:
...
def get_oder():
    ...
    order_craft = Craft(d_id='order', m_id='order', target=(orderML, 1), changeTime=0, line_id=0)
    return orderML, order_craft


# 形成有向无环图
# edge = {}
# count = {s_id:0 for s_id in STAGE_P_M}
# for stage in STAGE_P_M:
#     if not stage in edge:
#         edge[stage] = []
#     for m in STAGE_P_M[stage]:
#         material = list(MATERIAL[m].values())[0]
#         for source, _ in material.bom:
#             if source in M_T_STAGE:
