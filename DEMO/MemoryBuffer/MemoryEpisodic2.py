...
class MemoryBuffer:
    ...
    def append(self, experience):
        ...
        if experience['id'] in self.current.keys():
            ...
            np.append(self.current[experience['id']]
    ...
    def sample(self, idList: [], batchSize: int, mf=False, map=None):
        ...
        batch = {
            'obs': np.array([[] for _ in idList]),
            'action': np.array([] for _ in idList),
            'reward': np.array([] for _ in idList),
            'done': np.array([] for _ in idList),
            'next_obs': np.array([] for _ in idList),
            'next_avail_action': np.array([] for _ in idList),
            'last_iobs': np.array([] for _ in idList),
            'iobs': np.array([] for _ in idList),
            'mean_action': np.array([] for _ in idList),
            'last_mean_action': np.array([] for _ in idList),
        ...
        for item in range(batchSize):
            ...
            for index, d_id in enumerate(idList):
                ...
                if mf:
                    ...
                    one_hot = th.zeros((len(self.buffer[b_id][d_id]['action_index']), len(
                        self.buffer[b_id][d_id]['available_action'][0]))).to(device)
                    action_index = th.LongTensor(
        ...
        if mf:
            for index, d_id in enumerate(idList):
                for item in range(batchSize):
                    ...
                    batch['mean_action'][index].append(
                        copy.deepcopy(mean_action))
                    batch['last_mean_action'][index].append(
                        th.zeros((1, mean_action.shape[-1])).to(device))
                    batch['last_mean_action'][index].append(
                        copy.deepcopy(mean_action[:-1]))
                batch['mean_action'][index] = th.cat(
                    batch['mean_action'][index], dim=0).to(device)
                batch['last_mean_action'][index] = th.cat(
                    batch['last_mean_action'][index], dim=0).to(device)
            batch['mean_action'] = th.stack(
        ...
        batch['next_avail_action'] = th.FloatTensor(
            batch["next_avail_action"]).to(device)
    ...
    def get_current(self, d_id, item,  map=None):
        ...
        if item == 'last_mean_action':
            if d_id in self.current:
                mean = th.zeros(
                    len(self.current[d_id]['available_action'][-1]))
                for nei in map[d_id]['Counterparts']:
